...
trigger.* event
  ...
  → TaskRouter.admit(task, spec)
      ...
      └─ some invariant blocks → task PENDING → PendingQueue (deadline = now + TTL)
...
TaskRouter.release(task_id) / periodic tick
  → re-check queue in FIFO order:
      ...
      └─ deadline passed → dequeue → FAILED + routing.expired
```
...
| `router/queue.py` | `PendingQueue` — FIFO of `(task, spec, deadline)`; pure data structure, no I/O, no clock (caller passes `now`) |
...
- Blocked at admit → `routing.queued` (task_id, reason, deadline).
