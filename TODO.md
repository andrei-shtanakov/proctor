...
- ✅ Phase 3 (часть 3) — docker worker: core-managed container fleet (DockerWorkerManager + ContainerRuntime docker/podman CLI wrapper), fresh-id fencing, poll-loop restart (backoff/jitter/stability-reset/ceiling), Dockerfile + base worker config, integration test за `docker` pytest-маркером
