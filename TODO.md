...
- [x] **Guard результата MCP-инструмента в дизайне `mcp/`** (slug: mcp-tool-result-guard) @owner:github:andrei-shtanakov @id:mcp-tool-result-guard @epic:eco.distributed-execution — реализован `d911b0c` (`src/proctor/mcp/result_guard.py`, guard только детектирует; политика block/redact/log — за будущим mcp/proxy)
